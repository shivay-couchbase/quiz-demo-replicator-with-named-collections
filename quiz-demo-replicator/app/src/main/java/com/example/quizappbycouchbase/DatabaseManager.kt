...
class DatabaseManager(private val context: Context) {
    ...
    private val defaultDatabase = "quizapp"
    private val defaultImageCollection = "images"
    ...
    private val userScoresDBName: String = "scores"
    private var replicatorExecutor: ExecutorCoroutineDispatcher? = null
    ...
    fun initializeDatabases(){
        ...
        collection = database!!.getCollection("users","quizapp") ?: database!!.createCollection("users","quizapp")
        questionsCollection = database!!.getCollection("questions","quizapp") ?: database!!.createCollection("questions","quizapp")
        scoresCollection = database!!.getCollection("scores","quizapp") ?: database!!.createCollection("scores","quizapp")
        imagesCategoryCollection = database!!.getCollection(defaultImageCollection,"quizapp") ?: database!!.createCollection(defaultImageCollection,"quizapp")
        replicatorExecutor = Executors.newSingleThreadExecutor().asCoroutineDispatcher()
        GlobalScope.launch(replicatorExecutor!!) {
...
//        val collectionConfig = CollectionConfigurationFactory.newConfig(conflictResolver = LocalWinsResolver)
        val repl = Replicator(
            ...
            ReplicatorConfigurationFactory.newConfig(
                target = URLEndpoint(URI("wss://pjhdkgfki6jwmfz.apps.nonprod-project-avengers.com:4984/quizapp")),

                collections = mapOf(database!!.getCollections("quizapp") to null),
                ...
                authenticator = BasicAuthenticator("shivay", "Shivay123#".toCharArray())
            )
        )
...
//            val database: Database = openOrCreateDatabase()
            ...
            for (i in 0 until jsonArray_image.length()) {
                ...
                val mutableDocument = MutableDocument(unique_id.toString())
    ...
    fun search(image: List<Double>) : List<ImageObject> = synchronized(lock) {
        ...
        if (query == null) {
            val sql = "SELECT unique_id, name, imagevect_l2, category " +
                    "FROM quizapp:$defaultImageCollection " +
